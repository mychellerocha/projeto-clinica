...
<body>
    ...
    $result_consulta_cont = "SELECT * FROM consulta WHERE data_consulta = date(now()) ORDER BY horario ASC";
    ...
    echo '<div>'; 
        echo '<div class="tabela">';
            echo '<div style="float:right;">';
                echo '<a href="index-nova.php" class="botao flutuante">+ &nbsp Nova consulta</a>';
            echo '</div>';
            echo '<table>';
                if (empty($dados['pesqUsuario'])) {
                    if (($resultado_consulta_cont) and ($resultado_consulta_cont->rowCount() != 0)) {
                        echo '<h1>Consultas do dia</h1><br>';
                        echo '<tr>';
                            echo '<th>Paciente</th>';
                            echo '<th>Médico</th>';
                            echo '<th>Data</th>';
                        ...
                        while ($row_consulta_cont = $resultado_consulta_cont->fetch(PDO::FETCH_ASSOC)) {
                            ...
                            echo '<tr>';
                                ...
                                echo "<td><a href='index-edita.php?id=" . $row_consulta_cont['id'] . "'>Editar&nbsp&nbsp&nbsp&nbsp</a>";
                                echo "<a href='index-apaga.php?id=" . $row_consulta_cont['id'] . "'>Apagar</a></td>";
                            echo '</tr>';
                        }
                    } 
                    else {
                        echo '<h3>Não há consultas marcadas para hoje!</h3><br>';
                    }
                }
            echo '</table>';
        echo '</div>';
